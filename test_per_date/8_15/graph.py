...
async def create_graph(llm, tools):
    ...
    graph.add_node("move_agent", wrap_async(move_agent,  tool_node=tool_node))  # 단순 이동만 담당하는 에이전트
    ...
    graph.add_node("used_object_define_agent", wrap_async(used_object_define_agent, llm=llm, tool_node=tool_node))  # detect_failed_feedback_agent
    graph.add_node("assembly_agent", wrap_async(Assembly_agent, llm=llm, tool_node=tool_node))  # detect_failed_feedback_agent


    graph.add_edge("used_object_define_agent", "detectorAgent")
