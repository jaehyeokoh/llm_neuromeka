...
class AgentState(TypedDict):
    ...
    # execlude_boxes_img : Optional[List[str]] # used_object_define_agent가 필요없는 박스 제거한 base64이미지와 남은 박스들 [image64, boxes]
...
async def used_object_define_agent(llm, state:AgentState,tool_node, config=None)->dict:
    """ 
...
async def vlm_dino_finding_agent(state: AgentState, tool_node, config=None) -> Dict[str, Any]:
    ...
    # 1) bytes → str (BOM 포함시 자동 제거하려면 utf-8-sig)
    ...
    if isinstance(sopi, str):
        ...
        if s:
            try:
                ...
                # 필요하면 로깅하고 기본값
                print("[selected_objects_per_input] JSON decode error:", e)
                selected_objects_per_input = {}
        else:
            selected_objects_per_input = {}
        ...
        raise TypeError(f"selected_objects_per_input unexpected type: {type(sopi).__name__}")
...
######################################################################################################3
    ...
    for item in result_list:
        ...
        if isinstance(item, str):
            ...
            raise TypeError(f"side_point_clouds element unexpected type: {type(item).__name__}")
...
############################################################ anygrasp 영역 ##################################################
    ...
    for attempt in range(1, max_retries + 1):
        print(f"[anygrasp Agent] Tool call attempt {attempt}/{max_retries}")
        try:
    ...
    ####################################################################################################
            print(f"[anygrasp Agent] gg (len={len(gg)}).")
            if len(gg) <= 5:
                print(f"[anygrasp Agent] gg too few (len={len(gg)}). Retrying...")
                # 재시도 남아있으면 다음 반복으로, 없으면 에러
                if attempt == max_retries:
                    raise RuntimeError("gg length <= 4 after max retries")
                await asyncio.sleep(0.2 * attempt)  # 짧은 백오프
                continue  #재시도
