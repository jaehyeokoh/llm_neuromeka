...
from detection_utils import detect_objects_with_retries
import asyncio
...
async def vlm_agent(
    ...
    msg = moveL_message([0.45,0.15,0.50,0])
    await tool_node.ainvoke({"messages": [msg]})

    object_user_want_name = state.get("object_user_want_name")
    if object_user_want_name != [] or all_objects_information != []: # 앞서 user_input_divider_agent가 객체 이름 탐지 못할 시에만 VLM 에이전트 실행
        print("[VLM agent ] all_objects_information is not blank :", all_objects_information)
...
async def detect_failed_feedback_agent(llm, state: AgentState, tool_node, called_agent = None, detect_failed_objects = None, target_object=None, config=None, index = 0) -> Dict[str, Any]:
    """
    ...
    if not failed_objects:
        ...
        if single_feedback == True: # 단 하나의 입력에 대한 물건을 찾을 때
            global all_objects_information
            print("[feedback of gripper] : target object is ", target_object)
            obj = next((item for item in all_objects_information if item['id'] == str(target_object)), None)
            if obj != None:
                closest_obj = find_closest_detection(det_list, obj["position"], target_object) # 8_6일자 버그 수정 : detlist = 타겟의 가장 최신 정보, obj["position"] = 기존 물체의 위치 정보 -> 기존 찍은 물체와 가장 가까운걸 detlist에서 찾아서 반환
                # if new_obj != None:
                #     return {
                ...
                if new_obj != None:
                        # 로봇팔 엔드툴 포즈
                    tool_pos = await _get_tcp_pose(tool_node)
                    return {"closest_obj":new_obj, "tool_pos":tool_pos}
                else:
                        # 로봇팔 엔드툴 포즈
                    tool_pos = await _get_tcp_pose(tool_node)
                    return {"closest_obj":closest_obj, "tool_pos":tool_pos}
...
############################################################# # detect만으로 못했을 때 VLM으로 다시 시도 루프################
    ...
    if failed_objects: # detect만으로 못했을 때 VLM으로 다시 시도
        ...
        return await detect_failed_feedback_agent(
            llm, state, tool_node ,called_agent="detect_failed_feedback_agent",detect_failed_objects= names, target_object= target_object, config=config, index=index)
    


async def detector_agent_without_llm(state: AgentState, tool_node, all_object_detect = True) -> Dict[str, Any]:
    ...
    if all_objects_information != []:
        return{
        "information_of_object": all_objects_information
    }
    ...
    counts: Dict[str, int] = {}
    for_side_cloudes = [] # 사이드 클라우드와 아이디 저장
    ...
    side_point_clouds = side_pc_res[0].content
    # print("[detect agent] side point cloude is :", side_point_clouds)
...
async def simple_user_input_analyze_agent(llm, state: AgentState, config=None) -> dict:
    ...
    if all_objects_information != []:
        objects_user_input = all_objects_information
    try:
...
async def grip_agent(llm, state:AgentState , tool_node, move_plan, config=None) -> dict:
    ...
    global single_feedback # feedback 스위치
    #######################################################
...
######################################################################################################3
    ...
    new_obj = None
    for single_det in det_list:
        new_obj = match_or_create_object(prev_list=all_objects_information, new_det=single_det, tool_pos=None) # 기존에 감지된 물체와 증복되는지 확인 용
        print(f"[gripper agent] match_or_create_object result: {new_obj}")
        
    if new_obj is not None:  # 새로운 물체 발견 시
        all_objects_information.insert(0, new_obj)  # 리스트 맨 앞에 삽입
    ...
    obj = next((item for item in objects if item['id'] == str(target_object)), None)
    if obj and det_list != []: # 타겟 물체가 저장되어 있다면
        closest_obj = find_closest_detection(det_list, obj["position"], target_object) # 8_6일자 버그 수정 : detlist = 타겟의 가장 최신 정보, obj["position"] = 기존 물체의 위치 정보 -> 기존 찍은 물체와 가장 가까운걸 detlist에서 찾아서 반환
        print("[grip_agent]  found closest obj :", closest_obj)
        

    # print("[grip_agent] gripper's closest obj", closest_obj["id"], "gripper's closest obj end")
    # print("[gripper agent] det list vs closest obj :", det_list, "closesr :", closest_obj)
    tool_pos_feedback = tool_pos
    # 타겟이랑 closest_obj랑 다르면 피드백 함수 호출
    if closest_obj == [] or re.sub(r'[^a-zA-Z]', '', str(closest_obj["object_name"])).lower() != re.sub(r'[^a-zA-Z]', '', str(target_name)).lower(): # 기호 및 대소문자, 공백 제거 후 이름 비교
        print("[gripper agent]target is not detected, trying to feedback with detect_failed_feedback_agent")
        single_feedback = True
        feedback_message = await detect_failed_feedback_agent(llm,state,tool_node,"grip_agent",str(target_name), target_object, config=config,index = 0) # single_feedback = 하나만 받으려고 할 때
        ...
        if feedback_message == "done": # 만약 피드백 해도 못찾으면 그냥 리턴
            ...
            closest_obj = feedback_message["closest_obj"]
            tool_pos_feedback = feedback_message["tool_pos"]
...
######################################################## 마스크  추출 및 point cloud 추출 ##############################3
    ...
    last_tool_pos = tool_pos_feedback
    # 전체 포인트 클라우드 생성 메시지
    cloud_msg = generate_integrated_point_cloud_message(side_point_clouds, closest_obj["id"], closest_obj["mask_base64"], closest_obj["position"][2], tool_pos[:3],tool_pos_feedback) # 순서대로 앞서 저장한 전체 물체 보간된 클라우드, 타겟 id, 타겟 mask, 타겟 높이, 현재 툴 포즈
...
async def release_agent(tool_node, move_plan) -> dict:
    ...
    parsed = parse_result_list(side_point_clouds)
    updated = apply_offset_and_reencode(parsed, releasing_object, tool_pos_side_pc)
