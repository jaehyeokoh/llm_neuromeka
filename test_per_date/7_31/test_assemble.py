...
object_names = ["white box", "black object", "blue silver can" ]
...
def filter_objs_dimensions_height_name(objs):
    g = lambda d,*ks: next((d[k] for k in ks if k in d), None)
    pos = lambda p: ([float(p[0]),float(p[1]),float(p[2])] if isinstance(p,(list,tuple)) and len(p)>=3
                     else ([float(p['x']),float(p['y']),float(p['z'])] if isinstance(p,dict) and all(k in p for k in('x','y','z')) else None))
    nums = lambda v: [float(x) for x in v] if isinstance(v,(list,tuple)) else None
...
def parse_eval_refine_output_loose(text):
    ...
    # 2) 기본 패턴(A: ### 포함 / B: ### 없음)
    m = (re.search(r"evaluation:\s*(?P<eval>.*?)\n\s*###\s*\n\s*revised_plan:\s*(?P<plan>.*)\Z", s, flags)
         or re.search(r"evaluation:\s*(?P<eval>.*?)\n\s*revised_plan:\s*(?P<plan>.*)\Z", s, flags))

    # 3) 예시 섞임 대비: 마지막 페어 선택
    if not m:
        it = list(re.finditer(
            r"evaluation:\s*(?P<eval>.*?)"
            r"(?:\n\s*###\s*\n\s*)?"
            r"revised_plan:\s*(?P<plan>.*?)(?=\n[A-Za-z_]+:|\Z)", s, flags))
        if not it:
            raise ValueError("Could not find evaluation/revised_plan sections.")
        m = it[-1]

    ev, rp = m.group("eval").strip(), m.group("plan").strip()
    if not ev:
        raise ValueError("Empty evaluation section.")
    if not rp:
        rp = "No changes needed."

    # 4) LLM 재투입용 정규화
    def norm(x):
        x = x.strip()
        if (x[:1] in "\"'" and x[-1:] == x[:1]):  # 양끝 따옴표 제거
            x = x[1:-1]
        return (x.replace("\\r\\n", "\n").replace("\\r", "\n")
                    .replace("\\n", "\n").replace("\\t", "\t")
                    .replace("\r\n", "\n").replace("\r", "\n").strip())
    ev, rp = norm(ev), norm(rp)

    return {"evaluation": ev, "revised_plan": rp}
...
async def assemble_agent_llm(detect_data, tool_node,llm=llm):
    filtered_obj = filter_objs_dimensions_height_name(detect_data)
    ...
    # system_prompt = f"""
    # WHO YOU ARE:
    # you are a agent who can only pick N place

    # GOAL:
    # your goal is to make a plan to assemble that user want by provided object info (name, Dimenstion, height)

    # INPUT:
    # the user_input,
    # the objects information : {filtered_obj}

    # RULES:
    # 1. DO not consider physical 요소
    # 2. YOU can't use same object multiple times because the object is only one and it can't duplicated

    # HOW TO REASONING:
    # 1. describe the product's information of user_want
    # 2. find feature of assembled product
    # 3. describe the object you got
    # 4. make a step to assemble or build
    # 5. describe each step why it came
    # 6. find the proper object of each step
    # 7. make a plan to perform
...
INPUT:
- user_input: a description of the structure the user wants to build (e.g., dolmen, bridge, arch, etc.)

RULES:
- Do NOT use specific object names.
- Instead, describe what kind of objects are needed in terms of shape, size, flatness, or matching height.
...
HOW TO REASONING:
1. Explain what the requested structure is and its basic concept.
...
"""
    ...
    res = res2.content
    print("res2 is :", res)
    res2_parsed = parse_eval_refine_output_loose(str(res2))
...
- Gap condition: gap < capstone span (capstone can span both supports).
...
- upright, parallel, flat top, similar height
- gap smaller than (or <) capstone span/length/width
- capstone horizontally, centered
- simultaneously contacts both flat tops
- small translations/rotations
