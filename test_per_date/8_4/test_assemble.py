...
"""
    ...
    # shape_added_object_list = add_shape_from_id(filtered_obj) # 이름에 따라 매핑을 통한 모양 추출해 넣는 함수
    ...
    """
    # prompt_match_object = prompt_match_object.replace("{shape_added_object_list}",shape_added_object_list)
    # print("prompt match object is : ", prompt_match_object)
    res4 = llm.invoke([
...
- distance: specific measurement (e.g., 5cm, 10mm), or object-based measurement (e.g., obj3's longest axis, obj1's width) or None
...
EXAMPLE:
Input: Place beam1 horizontally across block1_0 and block2_0, spaced from the first to form a gap smaller than the white_box_0’s span.
...
STEP 2
analysis: Place beam1 horizontally across block1
target_object: beam1
reference_object: block1
relative_position: above
distance: None
object_orientation: parallel
