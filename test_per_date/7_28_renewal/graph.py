...
async def create_graph(llm, tools):
    ...
    def route_divide(state: AgentState):
        ...
        print(f"[Route_divider]임무가 배정된 에이전트:{agent}, 임무 내용 : {user_input_divide} ")
        state["user_input"] = user_input_divide
        state["done"] = False

        actions_divide_num += 1
        print(actions_divide_num)
        return {"result_of_divide_seq": {"result":"not_done", "agent":agent}, "user_input": user_input_divide, "done":False} # done도 False로 틀어야 다음에 실행됨
