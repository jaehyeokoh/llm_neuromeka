...
#     # 2. actions 파싱
#     actions_block = parts[1]
#     action_lines = actions_block.splitlines()
#     for line in action_lines:
#         match = re.match(r"^\s*(\d+)\.\s+(.*)$", line)
#         if match:
#             result["answer"].append(match.group(2).strip())
...
def match_or_create_object(prev_list, new_det, tool_pos, distance_threshold=0.04, position_only_check=True):
    """
    개선된 객체 매칭 함수
    ...
    if position_only_check:
        ...
        print(f"[MATCH] Checking name+position duplicates for {len(candidates)} objects with name '{name}'")
...
def remove_target_from_detections(detections: list, target_obj: dict, tolerance: float = 1e-3):
    def is_same_object(d):
        try:
            ...
            name1 = d.get("object_name", "").strip().lower()
            name2 = target_obj.get("object_name", "").strip().lower()
            if name1 != name2:
                return False

            pos1 = [round(float(p), 5) for p in d.get("position", [])]
            pos2 = [round(float(p), 5) for p in target_obj.get("position", [])]
            ...
            print(f"[ERROR] Object comparison failed: {e}")
            return False
