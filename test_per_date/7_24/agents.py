...
async def grip_agent(llm, state:AgentState , tool_node, move_plan, config=None) -> dict:
    ...
    # all_object_names = [n.strip().strip('"').strip("'") for n in all_object_names1 if n.strip()]
...
#### 1. 타겟 포함 주변 물체 vlm으로 감지해서 옆면이 보간된 point cloud를 생성 및 anygrasp 서버 실행해 파지점 추출 #################################
    ...
    res = json.loads(vlm_res) # str로 출력된 json의 형식을 str-> json으로 바꿔서 json 파싱이 가능하게 만들어줌 
    vlm_text = res["answer"] # 출력은 2가지로 나옴 1. chain_of_thought(추론단계 -> 이게 있어야 성능 향상), 2. answer -> 이게 최종 출력
    print("vlm의 파지점 주변 물체들 감지 출력:", vlm_text)

    # # vlm 출력에서 쓸대없는 기호들 제거하는거
    # cleaned_names = [
    # n.strip().strip('"').strip("'")
    # for n in (
...
####################################################################################################
        ...
        try:
            ...
            tilt_deg_threshold = 58 # 수평면으로부터의 x도 threshold  
            ...
            if len(low_tilt_grasps) >= len(high_tilt_grasps):
                ...
                secondary_grasp_height = grasp_height - 0.015 if grasp_height >= 0.09 else grasp_height - 0.015
            else:
                primary_list = high_tilt_grasps
                secondary_list = low_tilt_grasps
                # 수직(기울기 큼): 3cm 아래 (단, 최소 높이 보장)
                primary_grasp_height = grasp_height - 0.015 if grasp_height >= 0.09 else grasp_height - 0.015
                # 수평(기울기 작음): 4cm 아래 (단, 최소 높이 보장)
            ...
            if successful_grasp is None:
                ...
                try:
                    ...
                    # 만약 접근 위치의 z가 10 이하면 +5cm 해줌
                    offset = -0.08 * tool_z
                    if pos1[2] + offset[2] <= 0.09:
                        height_offset = 0.05
                    else:
                        height_offset = 0
