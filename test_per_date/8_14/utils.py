...
#             raise TypeError(f"parsed_objects는 list[dict] 또는 dict여야 합니다. got: {type(obj_dict)}")

#         if target_object_id not in obj_dict:
#             continue

#         encoded_data = obj_dict[target_object_id]
#         if encoded_data is None:
#             if skip_if_none:
#                 print(f"[INFO] {target_object_id} has None, skipped")
#                 continue
#             raise ValueError(f"{target_object_id} value is None")

#         try:
#             b = base64.b64decode(encoded_data)
#             if len(b) % (4*3) != 0:  # 3 floats(x,y,z) = 12 bytes
#                 print(f"[WARNING] byte length {len(b)} not multiple of 12 for {target_object_id}")
#             pts = np.frombuffer(b, dtype=np.float32).reshape(-1, 3)
#         except Exception as e:
#             print(f"[WARNING] decode/reshape failed for {target_object_id}: {e}")
#             continue
...
#         obj_dict[target_object_id] = new_b64
#         print(f"[INFO] {target_object_id}: offset applied ({len(pts)} pts)")

#     return parsed_objects

def apply_offset_and_reencode(
    parsed_objects: List[Dict[str, Any]] | Dict[str, Any],
    target_object_id: str,
    offset: Tuple[float, float, float],
    skip_if_none: bool = True,
) -> List[Dict[str, Any]]:
    ...
    for i, obj in enumerate(parsed_objects):
        # 1) 요소 정규화: str이면 JSON 파싱 + 리스트에 되돌려쓰기
        if isinstance(obj, str):
            try:
                obj = json.loads(obj)
            except json.JSONDecodeError as e:
                print(f"[WARN] items[{i}] JSON decode failed: {e}; skip")
                continue
            parsed_objects[i] = obj

        if not isinstance(obj, dict):
            print(f"[WARN] items[{i}] is {type(obj)}, expected dict; skip")
            continue

        # 2) 대상 id 없으면 패스
        if target_object_id not in obj:
            continue

        enc = obj[target_object_id]
        if enc is None:
            if skip_if_none:
                print(f"[INFO] {target_object_id} is None, skipped")
                continue
            raise ValueError(f"{target_object_id} value is None")
        ...
        try:
            ...
            if len(b) % 12 != 0:
                print(f"[WARN] byte length {len(b)} not multiple of 12 for {target_object_id}")
            pts = np.frombuffer(b, dtype=np.float32).reshape(-1, 3)
        except Exception as e:
            print(f"[WARN] decode/reshape failed for {target_object_id}: {e}")
            continue

        pts = pts + off
        obj[target_object_id] = base64.b64encode(pts.astype(np.float32).tobytes()).decode("ascii")
        print(f"[INFO] {target_object_id}: offset applied ({len(pts)} pts)")
...
def apply_world_transform(d, tool_pos, nd=3):
    ...
    def one(t):
        ...
        x, y, z = p[:3]
        ox, oy = tool_to_world(tool_pos, x, y, 0)[:2]
        # except: ox, oy = x, y
...
def task_summary_en(data):
    """
    Goal : when action block is more then 1, this func makes llm define left over objects
    Build one English sentence per task from {task: [ {id, purpose, ...}, ... ]}.
    - De-duplicates IDs per task, preserves first-seen order.
    - Formats each item as "id (purpose)" when purpose exists, else "id".
    - Returns newline-joined sentences; never truncates content.
    """
    output_sentences = []
    ...
    for task, items in data.items():
        ...
        if isinstance(items, list):
            for item in items:
                # 아이템이 딕셔너리 형태가 아니면 건너뜁니다.
                if not isinstance(item, dict):
                    continue
                
                item_id = item.get("object_name")
                
                # ID가 없거나 이미 처리한 ID이면 건너뜁니다.
                ...
                # ID와 목적(purpose)을 명확히 분리하는 문자열 형식 생성
                if purpose:
                    parts.append(f"the item '{item_id}' (purpose: {purpose})")
                else:
                    parts.append(f"the item '{item_id}'")

        # 처리된 아이템이 있는지 확인하여 최종 문자열을 조립합니다.
        if not parts:
            used_string = "no items"
        elif len(parts) == 1:
            used_string = parts[0]
        else:
            # 여러 아이템일 경우 쉼표와 'and'를 사용하여 자연스럽게 연결
            used_string = ", ".join(parts[:-1]) + f", and {parts[-1]}"

        # 최종적으로 LLM에게 전달될 자연스러운 문장을 생성합니다.
        # 아이템 개수에 따라 동사(was/were)를 올바르게 사용합니다.
        verb = 'was' if len(parts) <= 1 else 'were'
        output_sentences.append(f"For the task '{task}', {used_string} {verb} used.")
