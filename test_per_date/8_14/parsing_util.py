...
# 추가: 맨 앞 "숫자." 제거
LEADNUM_DOT = re.compile(r"^\s*\d+\.\s*")

def _strip_leading_number_dot(s: str) -> str:
    return LEADNUM_DOT.sub("", s).strip()

def parse_simple_plan_editor_output(text: str) -> dict:
    """
    reasoning/answer 하이브리드 포맷 파서.
    - answer 블록에서 앞 번호('1.', '2.' ...) 제거
    - 'release'가 나올 때마다 블록을 끊어 blocks로 반환
    """
    reasoning_lines, answer_lines = _split_reasoning_answer(
        text, missing_sep_msg="Output missing '###' separator"
    )

    # 1) key-value lists가 있으면 그대로 리턴(이 케이스는 기존 유지)
    output_dict = _parse_kv_lists(answer_lines)
    if output_dict:
        return {"reasoning": reasoning_lines, "answer": output_dict}
    ...
    # 3) 앞 번호 제거 + 공백 제거
    normalized = [_strip_leading_number_dot(ln) for ln in content_lines if ln.strip()]

    # 4) release 기준 블록 분할(대소문자 무시, 단어 경계)
    blocks = []
    cur = []
    for ln in normalized:
        cur.append(ln)
        if re.search(r"\brelease\b", ln, flags=re.I):
            blocks.append(cur)
            cur = []
    if cur:  # 마지막에 release가 없으면 꼬리 블록으로 추가
        blocks.append(cur)

    # 호환성: 기존 answer(리스트)도 유지하고, 새 키로 blocks를 추가
    return {"reasoning": reasoning_lines, "answer": normalized, "blocks": blocks}
