...
def generate_side_point_clouds_for_storage_message(objects_data, tool_pos):
    ...
    content="",
    tool_calls=[{
        "name": "generate_side_point_clouds_for_storage",
        "args": { "objects_data": objects_data,
                "tool_pos": tool_pos},
        "id": str(uuid4()),
        "type": "tool_call"
        }]
...
def generate_integrated_point_cloud_message(stored_side_clouds_str, target_object_id, target_mask_base64, target_obj_height, current_tool_pos,tool_pos_feedback):
    msg = AIMessage(
    content="",
    tool_calls=[{
        "name": "generate_integrated_point_cloud",
        "args": {
...
def robot_custom_move_message(x=None,y=None,z=None,r=None,p=None,w=None):
    msg = AIMessage(content="", tool_calls=[{
    "name": "robot_custom_move",
    "args": {"x":x,"y":y,"z":z,"r":r,"p":p,"w":w},
    "id": str(uuid4()),
    "type": "tool_call"
    }])
    return msg
...
def capture_dino_labeled_image_message():
    msg = AIMessage(content="", tool_calls=[{
    "name": "capture_dino_labeled_image",
    "args": {},
    "id": str(uuid4()),
    "type": "tool_call"
    }])
...
def get_3d_properties_from_boxes_message(boxes_coords):
    msg = AIMessage(content="", tool_calls=[{
    "name": "get_3d_properties_from_boxes",
    "args": {"boxes_coords":boxes_coords},
    "id": str(uuid4()),
    "type": "tool_call"
    }])
    return msg
...
def clean_keep_outer_braces(text):
    # '''로 둘러싸인 경우 제거
    if text.startswith("'''") and text.endswith("'''"):
        text = text[3:-3]

    # 첫 번째 { 와 마지막 } 포함한 부분 추출
    start = text.find('{')
    end = text.rfind('}')
    if start != -1 and end != -1 and start < end:
        return text[start:end+1]  # 중괄호 포함
    else:
        return None  # 중괄호가 없거나 잘못된 경우
...
def match_or_create_object(prev_list, new_det, tool_pos, distance_threshold=0.04):
    ...
    if candidates:
        ...
        if best_dist <= distance_threshold:
            ...
            # 기존 객체의 ID를 사용해서 새 객체 생성
            new_obj = {
                "id": best["id"],  # 기존 객체의 ID 사용
                "object_name": name,
...
def find_closest_detection(raw: str, old_pos: dict, id):
    """
...
def apply_plan_offset(direction, pos, offset_distance=0.15, grasp_z = 0.07):
    """
...
def deduplicate_by_position(objects, threshold=0.006):
    """
...
def filter_objs_dimensions_height_name_area(objs):
    ...
    for o in objs:
        ...
        out.append({
            ...
            # 'object_name': g(o,'id','object_name'),
            # 'height': p[2],
...
def convert_plan_to_commands(parsed_plan: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for step in parsed_plan:
        ...
        if isinstance(ref_list, list) and len(ref_list) == 2 and orient == "parallel":
...
# 릴리즈 에이전트 위치 업데이트 인코딩 디코딩용

def parse_result_list(mixed: Union[str, List]) -> List[Dict]:
    """문자열 또는 리스트 입력을 받아 내부 str 딕셔너리까지 2차 파싱한 리스트(dict들) 반환"""
    if isinstance(mixed, str):
        outer = json.loads(mixed)
    elif isinstance(mixed, list):
        outer = mixed
    else:
        raise ValueError("stored_side_clouds_str must be string or list")
...
def apply_offset_and_reencode(parsed_objects: List[Dict],
                              target_object_id: str,
                              offset: Tuple[float, float, float],
                              skip_if_none: bool = True) -> List[Dict]:
    """parsed_objects의 특정 id에 대해 디코드->오프셋 적용->재인코딩 후 교체"""
    ...
    for obj_dict in parsed_objects:
        for obj_id, encoded_data in list(obj_dict.items()):
            ...
            # 오프셋 적용
            pts = pts + off  # in-place 원하면: pts += off
...
def parse_to_dict_for_dino(raw_string: str) -> dict:
    """
    작은따옴표와 np.int64를 처리하여 문자열을 파이썬 딕셔너리로 변환.
    dino only의 출력에 numpy와 작은 따음표가 있어 json.loads가 안되던 문제 해결
    ...
    Returns:
        dict: 파싱된 딕셔너리.
    """
    # 작은따옴표를 큰따옴표로 변경, 'np.int64('와 ')'를 제거하여 JSON 표준에 맞춤
    cleaned_str = raw_string.replace("'", '"').replace("np.int64(", "").replace(")", "")
