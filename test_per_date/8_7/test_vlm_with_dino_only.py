...
async def vlm_dino_finding_agent(
    ...
    user_input = "후레쉬베리를 적절한 흰 상자에 넣어"
    # 3. Invoke LLM
    # prompt = "이건 수직으로 아래로 찍은 사진이야 고인돌 모양을 만들기 위해 필요한 객체의 번호를 reasoning:, answer:형식으로 말해줘"
    prompt = f"""
User_input : {user_input}

Goal: Describe the reasoning of which objects are necessary for user_input based on the overhead image. Identify the objects by their numbers.
...
"""
    ...
    # selected_boxes = boxes[i] # i는 vlm이 선택한 라벨 번호
    # sam_res = await tool_node.ainvoke({"messages": [get_3d_properties_from_boxes_message(selected_boxes)]})
