...
from utils import * # MCP 설정, 액션 포맷팅 같은 함수 불러오기
from AI_messages import * # tool message에 보낼 message 불러오기
from parsing_util import * # llm 출력 파싱 함수 불러오기
...
load_dotenv()
api_key = os.getenv("OPENAI_NEUROMEKA_API") # llm api key 불러오기

async def _get_tcp_pose(tool_node) -> list[float]:
    """ output : tcp's  [x,y,z, r, p w ]
        warining ! always use with 'await'
        robot의 tcp pos를 구하는 함수를 짧게 하기 위해 만든 거임 이거 호출하면 바로 결과 사용하면 됨
    """
...
이 코드는 아마추어가 만들어 각종 버그와 누락이 있습니다. 만약 사용하신다가 제 코드에서 이상한 부분이 발견되면 
"내가 모르는 무언가가 있구나. 건드리지 말아야지"라는 생각보다는 "개발자가 이건 실수했구나"라는 마인드로 고치세요.
...
class AgentState(TypedDict):
    ...
    redifined_task : str # used_object_define_agent가 필요없는 물건 제거하고 새로 명령 재구축 하는거 -> 명령을 넣음
...
action_num = 0 # supervisor가 사용하는 전역 변수

debug = False # true = 포인트 클라우드 및 grasp point 시각화

detect_feedback_trial = 0 # detect_failed_feedback_agent의 시도 횟수

all_objects_information = [] # 모든 물체 정보들

last_gripped = None # 마지막으로 잡은 물체

last_gripped_angle = None # 마지막으로 잡았을 때의 각도

side_point_clouds = None # 전역으로 저장된 전체 물체 side point cloud, 형태:[{id1:base64 encoded side pc},{},{},..] 주의: 이거 파싱하는 코드 잘 보고 파싱할것 파싱이 좀 더럽게 됨

last_tool_pos = None # 마지막으로 잡았을 때의 툴 포즈

released_object_height = 0 # 놔둘때 놔둔 위치의 물체의 높이 (물체 위에 놔둔 경우)

user_input_original = None # 사용자가 입력한 입력 저장용 (아직 사용처는 따로 없음 좀더 확장하면 사용될듯)
...
async def user_input_divider_agent(llm, state: AgentState, tool_node, config=None, index=0) -> dict:
    ...
    2. simple agent에게 할당된 Divided된 명령과 관련된 물체들을 object_user_want_name에 저장 및 vlm 에이전트 패스, 물체 특성 관련된 명령이면 아직 GroundedSAM 이전이니 object_user_want_name = []로 하고 이게 빈 리스트일때만 VLM에이전트 작동 -> 8_15 제거함 vlm-dino agent만 쓰는게 아직까지는 성능 더 좋은듯
...
async def used_object_define_agent(llm, state:AgentState, config=None)->dict:
    """ 
    이전에 사용한 물건을 받아 (selected_objects_per_input) 만약 ~를 제외하고 나머지를 처리하는거면 나머지에 대한 거로 명령 바꿔줌
    ex ) 캔을 잡은 후 상자에 올리고 나머지를 큰것부터 순서대로 상자 오른쪽에 놔 -> 캔을 이전에 잡았으면 캔을 제외한 나머지를 큰거부터 ~ 로 바꿈
    """
    ...
    divided_user_input = state.get("divided_user_input")
    global user_input_original

    def extract_actions(blocks):
    ...
    generated_task_blocks = extract_actions(divided_user_input)

    if selected_objects_per_input == {}: # 처음이면 그냥 패스
        return

    task_items = task_summary_en(selected_objects_per_input) # 이전 task별로 사용한 물체 id와 purpose들 출력함

    def fill(t, **vals):
        """ 프롬포트 내에 여러개의 문자열들 치환하는거 도와주는 헬퍼"""
        for k, v in vals.items():
            t = t.replace("{%s}" % k, v if isinstance(v, str) else json.dumps(v, ensure_ascii=False))
        return t

    system_prompt = fill(
        USED_OBJECT_DEFINE_PROMPT,
        user_input_original=user_input_original,
        generated_task_blocks=generated_task_blocks,
        one_of_divided_user_input=one_of_divided_user_input,
    ...
    print("[used_object_define_agent]",res)
...
async def vlm_dino_finding_agent(state: AgentState, tool_node, config=None) -> Dict[str, Any]:
    ...
    llm = ChatOpenAI(
        ...
        extra_body={
            ...
            "reasoning": {"effort": "minimal"},     # minimal | low | medium | high ( 높을 수록 성능 향상 but 시간 오래 걸림) -> 떨어질거 같은 물체 구분하는건 최소 low 이상 이어야 가능
        },
        api_key=api_key,
    )
    ...
    sopi = state.get("selected_objects_per_input")
    redifined_task = state.get("redifined_task")

    if redifined_task != None:
        print(f"[vlm_dino_finding_agent] changed task {user_input} -> {redifined_task} ")
        user_input = redifined_task
        

    # 1) bytes → str 
    if isinstance(sopi, (bytes, bytearray)):
        sopi = sopi.decode("utf-8-sig", errors="ignore")

    if isinstance(sopi, str):
        s = sopi.lstrip("\ufeff").strip()
        if s:
            try:
                selected_objects_per_input = json.loads(s)
            except json.JSONDecodeError as e:
                # 필요하면 로깅하고 기본값
                print("[vlm_dino_finding_agent] JSON decode error:", e)
                selected_objects_per_input = {}
        else:
            selected_objects_per_input = {}
    elif isinstance(sopi, (dict, list)):
        # 이미 파싱된 구조면 그대로 사용
        selected_objects_per_input = sopi
    elif sopi is None:
        selected_objects_per_input = {}
    else:
        # 예상치 못한 타입이면 명시적으로 실패시키거나 기본값
        raise TypeError(f"[vlm_dino_finding_agent] selected_objects_per_input unexpected type: {type(sopi).__name__}")
    ...
    if resp_msgs:  # 감지된게 있다면
        ...
        print("[vlm_dino_finding_agent] boxes are:", boxes)
    ...
    if not props or not boxes:
        print("[vlm_dino_finding_agent] No valid image/boxes")
        return {"det_list": [], "selected_label2name": {}}


    # 2) VLM: 번호→이름 추출
    prompt = VLM_DINO_FINDING_PROMPT.replace("{user_input}",user_input)

    msg = HumanMessage(content=[
        {"type": "text", "text": prompt},
        {"type": "image_url", "image_url": {"url": props}},
    ])
    res = llm.invoke([msg], config=config) # 주의 ainvoke 쓰면 느려짐
    print(res)

    parsed = parse_llm_output(res)  # 내 파일의 함수: [number, object name, purpose] 이렇게 3개를 파싱
    ...
    for triplet in parsed.get("answer", []):
        ...
        if idx is None:
            print(f"[vlm_dino_finding_agent] skip invalid label {raw_num}")
            continue
    ...
    # (옵션) 디버깅 출력
    print(f"[vlm_dino_finding_agent] selected_idxs={selected_idxs}, unselected_idxs={unselected_idxs}")
    if det_list_all:
        print("[vlm_dino_finding_agent] sample det 0:", {k: det_list_all[0].get(k) for k in ("id", "position", "dimensions", "area")})
    ...
    print("[vlm_dino_finding_agent]", side_cloud_args)
    ...
    return {
        ...
        "one_of_divided_user_input" : user_input
    }
...
async def simple_user_input_analyze_agent(llm, state: AgentState, config=None) -> dict:
    ...
    사용자 입력인 user_input만 받음 and 추가로 물체의 id만 받음
    출력은 1. action 1, 2. action2, 3.~ 이런식
    """
    user_input = state.get("one_of_divided_user_input")
    ...
    try:
        ...
        print("[simple_user_input_analyze_agent] objects_user_input : ", objects_user_input, "one_of_divided_user_input :", user_input)
...
async def simple_plan_editor_agent(llm, state: AgentState, config=None) -> dict:
    """
    ...
    print("[simple_plan_editor_agent] result is :", res)
    parsed = parse_simple_plan_editor_output(res)
    print("[simple_plan_editor_agent] parsed is :", parsed)
...
async def analyzed_user_input_regenerate_supervisor(llm, state: AgentState, tool_node, config=None) -> dict:
    ...
    user_input_analyzed_by_LLM = state.get("user_input_analyzed_by_LLM")
    print("[analyzed_user_input_regenerate_supervisor] user input analyzed by llm:",user_input_analyzed_by_LLM,"len",len(user_input_analyzed_by_LLM) )

    global action_num # 액션이 끝나는걸 트리거 하기 위한 변수 (부여받은 user_input_analyzed~의 리스트 길이로 판단)
    bias = 0
    # hard agent의 출력인 경우
    ...
    print("[analyzed_user_input_regenerate_supervisor] supervisor start with action num:", action_num+bias)
    user_input = user_input_analyzed_by_LLM[action_num+bias]
    if bias == 0:
        system_prompt = ANALYZED_USER_INPUT_REGENERATE_SIMPLE_PROMPT
        user_input = str(user_input)
        print("[analyzed_user_input_regenerate_supervisor] current action to execute:", user_input)
        res = llm.invoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content=user_input)
            ], config=config)
        ...
        if output.startswith("```") or output.endswith("```"): # 맨 앞뒤 ```가 있는 경우가 가끔 있어 이것만 잘라내고 다시 trim
            output = output[3:-3].strip()  

        keys = ("agent", "action", "target") # 각각 사용할 agnet, 수행할 action, 타겟 물체 or 장소 (llm 출력 파싱에 사용될 key)
        parsed = [
    ...
    for curr, nxt in zip_longest(parsed[bias:], parsed[bias+1:]):
        if curr["agent"] == "move":
            ...
            result = await grip_agent(state, tool_node,curr)
            ...
            action_num = 0
            await tool_node.ainvoke([robot_move_home_message()])
            return{
                "done" : True
            }
...
############################################################################################################################################################################ move agent
async def move_agent(state:AgentState , tool_node, move_plan, grip = False, release = False) -> dict:
    """
    ...
    입력 : move_plan = analyzed_user_input_regenerate_supervisor가 분배한 action, target등등이 포함된 명령, grip/release = grip/release 하기 전 액션인지에 대한 정보(bool)
    """
    offset_distance = 0.15 # 기본적인 z축 오프셋 (15cm 위에서 이동)
    print("tool_node in move_agent:", type(tool_node), getattr(tool_node, "keys", lambda: [])())

    # 로봇팔 엔드툴 포즈
    tool_pos = await _get_tcp_pose(tool_node)
    print("[move_agent] robot_arm_attached",tool_pos)
    ...
    if "home" not in str(target).strip() and "home" not in str(plan):
        print("[move_agent] moving before grip or release")
        ...
        if action_num == 0: # 첫번째 액션이라면
            ...
            print("[move_agent] edited_pos is : ", edited_pos)
            # 충돌 감지
            ...
            print("[move_agent] collision res is ",collision_res)
            if collision_res[0]:
                print("[move_agent] collision occured , margin is ", collision_res[1])
            ...
            if release:
                print("[move_agent] applying release offset")
                before_release_pos = edited_pos
                before_release_pos[2] = tool_pos[2] + collision_margin - 0.01
            ...
            if collision_res[0]:
                ...
                print("[move_agent] offset movement : ",offset_msg)
                edited_pos[2] = edited_pos[2] + collision_res[1]
                collision_margin = collision_res[1]
    ...
    else: # 이상한 action이 들어왔을 때 -> 일단 home pos로 가게 만듬

        print("[move_agent] go to default pose")
        await tool_node.ainvoke([move_default_pos_message()])
...
############################################################################################################################################################################ grip agent
async def grip_agent(state:AgentState , tool_node, move_plan) -> dict:
    """
    ...
    ################# 로봇팔 엔드툴 포즈 추출 ################
    tool_pos = await _get_tcp_pose(tool_node)
    print("[gripper agent] 엔드툴 pos : ",tool_pos)
    ########################################################
...
######################################################################################################3
    ...
    ############################## side point cloud 파싱 에러 잡는용도 
    sopi = side_point_clouds

    if sopi is None or sopi == "":
        result_list = []
    else:
        # bytes/bytearray → str (BOM 자동 제거)
        if isinstance(sopi, (bytes, bytearray)):
            sopi = sopi.decode("utf-8-sig", errors="ignore")

        # str → JSON 파싱 (앞쪽 BOM 방어)
        if isinstance(sopi, str):
            s = sopi.lstrip("\ufeff").strip()
            sopi = json.loads(s) if s else []

        # 이미 파싱된 구조
        if isinstance(sopi, dict):
            result_list = [sopi]
        elif isinstance(sopi, list):
            result_list = sopi
        else:
            raise TypeError(f"[gripper agent] side_point_clouds unexpected type: {type(sopi).__name__}")

    # 2차 파싱: 요소가 문자열이면 다시 JSON 파싱
    parsed_objects = []
    for item in result_list:
        if isinstance(item, (bytes, bytearray)):
            item = item.decode("utf-8-sig", errors="ignore")

        if isinstance(item, str):
            s = item.lstrip("\ufeff").strip()
            parsed_item = json.loads(s) if s else {}
        elif isinstance(item, dict):
            parsed_item = item
        else:
            raise TypeError(f"[gripper agent] side_point_clouds element unexpected type: {type(item).__name__}")
    ...
    if isinstance(bbox, str):
        bbox = json.loads(bbox)  # -> ["600","114","721","252"]꼴
    
    bbox = bbox[target_object] # 박스들 중 타겟의 박스 찾는거

    resp_debug = await tool_node.ainvoke([match_input_bboxes_with_dino_message([bbox])]) # sidecloud로 만든 박스의 정확도가 높지 않으므로 새로 dino돌려 기존 박스와 가장 유사한거 반환
    if resp_debug:  # 감지된게 있다면
        content_raw = resp_debug[0].content
        content_raw = parse_to_dict_for_dino(content_raw)   # parsing_util의 함수
        content = content_raw.get("image")
        bbox = content_raw.get("boxes", [])
        print("boxes debug are:", bbox)
        # if isinstance(content, str) and content.startswith("data:image/jpg;base64,"): # 디버깅용 저장 코드임 현재 카메라에 잡힌 이미지 보려면 활성화 할것
        #     props = content
            # save_base64_image(props)  # 디버깅용 저장 (현재 디렉토리에 저장됨)


    # 숫자로 캐스팅
    print("[gripper agent] bbox final is ", bbox)
    ...
    dets = update_world_coordinates(tool_pos, dets) # 로봇 툴 좌표 받아와 카메라 기준의 상대 좌표를 절대 좌표로 바꿈
    print(dets)
    target_obj_z = dets[0]["position"][2]
    if dets[0]["position"][2] <= 0.02: # 새로 계산된 타겟의 높이가 매우 낮으면 버그로 간주하고 이전에 저장한 값 사용
        print(f"[Gripper agent] the object height is too low{target_obj_z}, it might be a error, so change it to saved height")
        obj = next((item for item in objects if item['id'] == str(target_object)), None) # 타겟이 저장된 리스트에 있는지 확인하는 코드
        if obj: # 저장된 기존 타겟
            dets[0]["position"][2] = obj["position"][2]

    integrated_msg = generate_integrated_point_cloud_message(side, target_object, dets[0]["mask_base64"], dets[0]["position"][2], tool_pos[:3]) # 전체 포인트 클라우드 생성
    integrated_res = await tool_node.ainvoke([integrated_msg])
    cloud_res = integrated_res[0].content

    print("[gripper agent] cloud res is ",cloud_res[:80])
    try:
        cloud_res = json.loads(cloud_res)  # 리스트 안의 문자열들
        target_boundary_info = cloud_res["target_boundary_info"]
        print("[gripper agent] target boundary info:", target_boundary_info)

    except Exception as e:
        print("[gripper agent] 파싱중 오류남",e, "리턴된 클라우드: ", cloud_res)
...
############################################################ anygrasp 영역 ##################################################
    ...
    for attempt in range(1, max_retries + 1):
        print(f"[gripper agent] Tool call attempt {attempt}/{max_retries}")
        try:
            # print("target side pts cloud is ", target_boundary_info[:200])
            print(f"[gripper agent] Tool call attempt {attempt + 1}/{max_retries}")

            grasp_msg = anygrasp_message(cloud_res,target_boundary_info, debug)
            grasp_res = await tool_node.ainvoke([grasp_msg])
            print("grasp res is : ",grasp_res[0].content[:2000]) # debug
    ...
    ################################################ 시각적 디버그용 코드#####################
            if debug:
                ...
                o3d.visualization.draw_geometries([cloud_pc]) # 포인트 클라우드만 보이게 하는거
    ...
    ####################################################################################################
            print(f"[gripper agent] gg !!!!!!!!!!!!!!!!!!!!!!!(len={len(gg)}).")
            if len(gg) <= 7: # 7개보다 생성된 포인트 작으면 재시도
                print(f"[gripper agent] gg too few (len={len(gg)}). Retrying...")
                # 재시도 남아있으면 다음 반복으로, 없으면 에러
                if attempt == max_retries:
                    raise RuntimeError("gg length <= 4 after max retries")
                await asyncio.sleep(0.2 * attempt)  # 짧은 백오프 (0.2초)
                continue  #재시도
            ...
            try:
                ...
                all_grasp = []
                tilt_deg_threshold = 40 # 수평면으로부터의 x도 threshold  
                ...
                print(f"[gripper agent] 수평에 가까운 그래스프 (≤{tilt_deg_threshold}°): {len(low_tilt_grasps)}개")
                print(f"[gripper agent] 수직에 가까운 그래스프 (>{tilt_deg_threshold}°): {len(high_tilt_grasps)}개")
                ...
                async def try_grasp_list(grasp_list, list_name, adjusted_height):
                    ...
                    for grasp_data in grasp_list:
                        ...
                        score = i["score"]
                        print("[gripper agent] grasp score is ", score)
                        if score <= 0.05: # 점수 기준으로 필터링 (계속 튜닝 할것)
                            continue
                        
                        print(f"[gripper agent] 기울기 {tilt_from_xy:.2f}° 그래스프 시도 중...")
                        ...
                        if ik_res[0].content == "true" and ik_res2[0].content == "true":
                            print(f"[gripper agent] IK 체크 성공!! 기울기: {tilt_from_xy:.2f}°")
                            return grasp_data, i, adjusted_height
                        else:
                            print(f"[gripper agent] IK 실패 - 기울기: {tilt_from_xy:.2f}°")
                    
                    print(f"[gripper agent] {list_name} 모든 그래스프 실패")
                ...
                # 최대 7개까지 순서대로 기울기 확인
                for best_grasp in all_grasp[:7]:
                    try:
                        if abs(best_grasp['tilt']) >= 74: # 앞에서 점수 높은 7개 중 기울기 74도 이상인게 있으면 그거 우선 수행 
                            successful_grasp = best_grasp
                            g = best_grasp['grasp']
                            if grasp_height >= 0.09:
                                adjusted_height = grasp_height - 0.01 # 물체의 z축 기준으로 좌표 나오므로 1cm 아래로 내려서 그리퍼가 물체 잡게 함
                            elif grasp_height <= 0.03:
                                adjusted_height = 0.03 + abs(grasp_height - 0.03) # 3cm 미만의 물체면 오프셋 적용 (책상으로 놓은 나무 판자가 3cm라서 이렇게 한거임 안그러면 로봇이 책상에 밖음)
                            else:
                                adjusted_height = grasp_height
                            print(f"\n[gripper agent] IK 없이 선택된 그래스프 (수직 우선), 기울기: {best_grasp['tilt']:.2f}°")
                            break  # 성공했으면 루프 종료
                    except Exception as e:
                        print("[gripper agent] grasp 검사 중 에러:", e)
                ...
                if successful_grasp is None and len(primary_list) > 0:
                    ...
                    if successful_grasp is None and len(secondary_list) > 0:
                        print(f"\n[gripper agent] 우선순위 리스트 실패, 백업 리스트로 전환...")
                    ...
                    if successful_grasp is None:
                        print("\n[gripper agent] 모든 그래스프 포인트에서 IK 체크 실패")
                        # 모두 ik 실패하면 그냥 점수 가장 높은거로 강제 실행(with singular pass feature)
                        best_grasp = primary_list[0]
                        successful_grasp = best_grasp
                        g = best_grasp['grasp']
                        adjusted_height = primary_grasp_height
                        print(f"\n [gripper agent] 가장 점수 높은것 사용, 기울기: {best_grasp['tilt']:.2f}°")
                        # raise Exception("사용 가능한 그래스프 포인트가 없습니다.")
                    else:
                        print(f"\n[gripper agent] 최종 선택된 그래스프 기울기: {successful_grasp['tilt']:.2f}°")
                ...
                try:
                    ...
                    print("[gripper agent] resmove1 is", resmove1)
                    
                    # 전역변수 last_gripped_angle 에 잡았을 때의 각도 저장 -> 나중에 tilt agent에서 사용하려고 만든거임 지금 당장은 사용 안함
                    ...
                    if abs(tilt_from_xy) <= tilt_deg_threshold:  # 기울기가 수평면으로부터 58도 이하라면 그리퍼 재정렬
                        ...
                        print(f"[gripper agent] 그리퍼 재정렬됨: {new_rpy}")
                    else:
                        new_rpy = [pos1[3], pos1[4], pos1[5]]
                        print(f"[gripper agent] 재졍렬된 그리퍼 아닌 오리지널 값 사용: {new_rpy}")
                    ...
                    print("[gripper agent] resmove2 is", resmove2)
                    ...
                    print("[gripper agent] movel pos1:", target_pos)

                    grip_res = await tool_node.ainvoke([gripper_message("Grab")])
                    time.sleep(0.5)
                    print("[gripper agent] 그립 완료:", grip_res)
                    last_tool_pos = target_pos
                    
                    # 마지막으로 잡은 높이 저장
                    grasp_z = pos1[2] + 0.007
                    ...
                    print("[gripper agent] movel pos2:", target_pos2)
                    break
                except Exception as e:
...
async def release_agent(tool_node, move_plan) -> dict:
    """
    그리퍼 릴리즈 하는 에이전트
    릴리즈 후 해당 객체의 side point cloud 위치 업데이트 (단, 물체가 구르거나 놓은 위치에서 조금 이동하는 경우가 있으니 이점 유의할것)
    """
    global side_point_clouds
    global all_objects_information
    global last_tool_pos
    ...
    tool_pos_side_pc = [
        ...
        -(released_object_height)     # -Z (카메라 좌표계라 음수로 들어감)
    ]
    ...
    if sopi is None or sopi == "":
        result_list = []
    elif isinstance(sopi, (bytes, bytearray)):
        sopi = sopi.decode("utf-8-sig", errors="ignore")  # BOM 포함시 안전
        s = sopi.lstrip("\ufeff").strip()
        loaded = json.loads(s) if s else []
        result_list = loaded if isinstance(loaded, list) else [loaded]
    elif isinstance(sopi, str):
        s = sopi.lstrip("\ufeff").strip()
        loaded = json.loads(s) if s else []
        result_list = loaded if isinstance(loaded, list) else [loaded]
    elif isinstance(sopi, dict):
        result_list = [sopi]
    elif isinstance(sopi, list):
        result_list = sopi
    else:
        raise TypeError(f"side_point_clouds unexpected type: {type(sopi).__name__}")
    ...
    updated = apply_offset_and_reencode(side, releasing_object, tool_pos_side_pc) # 잡은 후 놔둔 물체의 side point cloud에 오프셋 적용하는 코드
    side_point_clouds = updated

    z = tool_pos[2] + 0.13
    await tool_node.ainvoke({"messages": [robot_custom_move_message(z=z)]}) # 놓은 후 위로 13cm 이동
    await tool_node.ainvoke({"messages": [move_default_pos_message()]}) # 기본 위치로 이동
    return


async def tilt_agent(tool_node, move_plan) -> dict:
    """
    그리퍼 릴리즈 하는 에이전트 (아직 제작 중)
...
async def hard_agent_supervisor(llm, state: AgentState, tool_node, config=None, index: int = 0) -> Dict[str, Any]:
    ...
    assign_output = llm.invoke([SystemMessage(content=assign_prompt)], config=config)
    ...
    if selected_agent == "Assembly_agent": # 일단 assembly만 만듬
...
async def Assembly_agent(llm, state: AgentState, tool_node, config=None, index: int = 0) -> Dict[str, Any]:
    ...
    system_prompt = ASSEMBLY_PLANNING_PROMPT
    res = llm.invoke([
        SystemMessage(content=system_prompt),
        HumanMessage(content=user_input)
    ], config=None)
