...
class GroundedSAM2Wrapper:
    def __init__(
        ...
        # sam2_config_path="configs/sam2.1/sam2.1_hiera_b+.yaml", # 경로 오류 뜨면 Grounded-SAM-2 폴더 내부 위치 넣기
        # sam2_ckpt_path = "Grounded-SAM-2/checkpoints/sam2.1_hiera_base_plus.pt", # 경로 오류 뜨면 Grounded-SAM-2 폴더 내부 위치 넣기
        device=None,
    ):
...
def cluster_and_generate_surface(pts: np.ndarray,
    ...
    for lbl in range(max_label + 1):
        ...
        # 벽면 보간
        side_pts, boundary_info = generate_side_point_cloud(cluster_pts,
                                             num_edge_samples=30,
...
async def generate_completed_point_cloud(
    ctx: Context,
    mask_base64_list: Union[List[str], Any] = None,
    mask_shape_list: Union[List[List[int]], Any] = None,
    obj_height_list: Union[List[float], Any] = None,
    camera_index: int = 0,
    eps: float = 0.005,
    min_samples: int = 10,
            ...
            for y in range(h):
                for x in range(w):
                    if not mask_np[y, x]:
                        continue
                    d = depth_m[y, x]
                    if not (0.01 < d < 0.8): continue # 감지 범위 = 카메라로부터 10cm ~ 80cm
                    X, Y, Z = rs.rs2_deproject_pixel_to_point(intrinsics, [x, y], d)
                    color = color_image[y, x].astype(np.float32) / 255.0
    ...
    for i in range(len(masked_pc_list)):
        ...
        pcd_add ,target_boundary = cluster_and_generate_surface(
            pts=pc_masked_np,
            cols=col_masked_np,
            eps=eps,
        ...
        # 타겟인 경우 후에 anygrasp에서 파지점 필터링 할 side영역 반환, 여기서 타겟은 약속했던 것처럼 리스트 마지막 물체
        # if i == len(masked_pc_list) - 1:  
        target_boundary_info = target_boundary

    # 5. 중심 기준 X 필터링 여기서 중심은 전체 포인트 클라우드 중심을 이야기 하는것 = 목표 물체 위에서 본다는 가정 하에 좌 우 잘라서 용량 감소
    x_vals = pc_all_np[:, 0]
    ...
    return {
        ...
        # "target_boundary_info": target_boundary_info # 타겟 추가 (7_23)
         "target_boundary_info": base64.b64encode(pickle.dumps(target_boundary_info)).decode() if target_boundary_info is not None else None
        # "target_boundary_info": None
    }
