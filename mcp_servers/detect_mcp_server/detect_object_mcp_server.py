...
from typing import  Dict, Any, List , Union # Type Hint
...
"""
사용된 카메라 : Intel D435i wrist camera -> 오프셋 : 0.09cm
...
"""
from mobile_sam import SamPredictor, sam_model_registry # SAM은 mobile sam 사용 -> 메모리 사용량이 적고 속도도 빠름
import logging
...
"""
# 전역 싱글톤 인스턴스들 (메모리 최적화 용 -> 한번 불러 온 후 여기에 저장해 사용해 사용할 때마다 불러오는 딜레이 감소)
_model_wrapper_instance = None
_camera_capture_instance = None

# 카메라가 기울어진 각도,  단위: 도(degree)
camera_tilt_theta = 0.0  # 카메라가 아래를 향해 기울어져 있는거로 고정 시 0으로 설정 
...
class RealSenseCapture:
    """ 사용된 카메라 : D435i, 카메라 위치 : wrist카메라, 그리퍼 중심으로부터 9cm 떨어짐"""
    ...
    def capture_aligned_frames(self, timeout_ms=5000):
        ...
        try:
            ...
            threshold_m = 0.8 # 0.8m 이상으로 먼거는 날림
...
def calculate_object_properties(mask, depth_image_raw, intrinsics, depth_scale): # added in 7_31 but PCA remains
    """
    마스크된 영역의 물체의 포인트 클라우드를 통해 3d 정보를 계산
    """
    try:
        # 1) 입력 검증 
...
async def process_single_object_mcp_optimized(
    ...
    단일 객체 이름, 이미지, depth등등 받아 sam으로 3d property 얻음
    """
...
def run_dino_inference_with_boxes(image_np_bgr, wrapper):
    ...
    try:
        ...
        if len(input_boxes) > 0:
            # === 박스 및 라벨에 사용할 색상 팔레트 정의 (BGR)(박스 색이 같으면 vlm이 다른 물체의 박스를 해당 물체의 것으로 착각해서 만든 거) ===
            color_palette = [
                (0, 0, 255),    # 빨강
                (0, 255, 0),    # 초록
                (200, 0, 0),    # 파랑
            ...
            for i, box in enumerate(input_boxes):
                ...
                # === 크기별 라벨 위치 결정 ===
                area_threshold = 5500 # 이거보다 픽셀 크기가 크면 라벨이 중앙에 생김
                ...
                if box_area > area_threshold:
                    ...
                    cv2.rectangle(annotated_frame, (bg_x1, bg_y1), (bg_x2, bg_y2), color, -1) # 라벨 감싸는 배경도 색을 따라가게 함
...
async def generate_integrated_point_cloud(
    ...
    # new_target_points = np.empty((0, 3)) #  꼭 지울것!!!! 테스트용으로 타겟 사이드 잠깐 빈값으로 덮어씌운거임 -> 이거 활성화 시 보간된 target side p.c 안생김
    ...
    if stored_points_list:
        ...
        print(f"[INFO] Added stored side points: {stored_points_count} (already in absolute coordinates)") # 주석 꼭 풀것 !!! 디버그 용으로 잠깐 저장된 사이드 포인트 비활성화 한거임
